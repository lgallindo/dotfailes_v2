# Documentation Index

This directory contains formal policies, standards, and historical records for the `dotfailes_v2` project.

## Policies & Standards

- [Git Flow Policy](GIT_FLOW.md): Defines branch management, staging, and integration procedures.
- [Commit Messages](COMMIT_MESSAGES.md): Strict Conventional Commits standard and scope definitions.

## Decision Records

- [ADRs Index](ADRs/README.md): Collection of Architectural Decision Records.

---
[Parent README](../README.md)
